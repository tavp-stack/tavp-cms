...
<div class="flex justify-between items-center mb-gutter">
  ...
  <a href="<?= $adminPrefix ?>/menus/create" class="bg-secondary text-on-secondary font-label-caps text-label-caps py-3 px-6 hard-step-shadow hover:brightness-110 active:translate-y-[1px] transition-all">+ NEW MENU</a>
</div>
      ...
      <?php else: foreach ($menus as $menu): ?>
        <tr class="border-t border-outline-variant hover:bg-surface-container-high/50 transition-colors">
          ...
          <td class="px-4 py-3 text-right">
            <a href="<?= $adminPrefix ?>/menus/<?= $this->e($menu['id']) ?>/edit" class="text-secondary font-label-caps text-label-caps hover:underline mr-3">Edit</a>
            <form method="post" action="<?= $adminPrefix ?>/menus/<?= $this->e($menu['id']) ?>/delete" class="inline">
