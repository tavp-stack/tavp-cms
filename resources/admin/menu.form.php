...
<div class="flex items-center gap-4 mb-gutter">
  <a href="/admin/menus" class="text-on-surface-variant hover:text-secondary transition-colors">
...
<form method="post" action="<?= $this->e($action) ?>" class="space-y-6 max-w-2xl">
      ...
      <div class="space-y-3">
        <?php foreach ($items as $item): ?>
          <div class="flex items-center gap-3 bg-surface-container-low border border-outline-variant rounded p-3">
            ...
            <form method="post" action="/admin/menus/<?= $this->e($menu['id'] ?? '0') ?>/items/<?= $this->e($item['id']) ?>/delete" class="inline">
  ...
  <div class="bg-surface-container border border-outline-variant p-6">
    ...
    <div class="flex gap-3">
      ...
      <input type="text" name="url" placeholder="/path" required
        class="flex-1 bg-surface-container-low border border-outline-variant rounded px-4 py-2 text-on-surface text-sm focus:border-secondary outline-none transition-colors">
      <button type="submit" formaction="/admin/menus/<?= $this->e($menu['id'] ?? '0') ?>/items" class="bg-secondary text-on-secondary font-label-caps text-label-caps py-2 px-4 hard-step-shadow hover:brightness-110 active:translate-y-[1px] transition-all">Add</button>
    </div>
  </div>
  ...
  <div class="flex gap-3">
    ...
    <a href="/admin/menus" class="border border-outline-variant font-label-caps text-label-caps py-3 px-8 hover:bg-surface-container-high transition-colors">CANCEL</a>
  </div>
</form>
