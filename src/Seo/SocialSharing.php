...
class SocialSharing
{
    public static function render(string $url, string $title, array $platforms = []): string
    {
        $platforms = $platforms ?: ['twitter', 'facebook', 'linkedin', 'whatsapp', 'telegram'];
        ...
        $html .= '<span class="text-gray-400 text-sm">Share:</span>';

        foreach ($platforms as $platform) {
            if (isset($links[$platform])) {
                $icon = $icons[$platform] ?? '';
                $html .= '<a href="' . $links[$platform] . '" target="_blank" rel="noopener noreferrer" ';
                $html .= 'class="text-gray-400 hover:text-white transition-colors" title="Share on ' . ucfirst($platform) . '">';
                $html .= $icon;
                $html .= '</a>';
            }
        }
